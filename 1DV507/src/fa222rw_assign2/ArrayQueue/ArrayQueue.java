...
public class ArrayQueue implements Queue {
	...
	private class arrIterator implements Iterator<Object> {
		...
		// used so the iterator does not terminate before it iterates anything. 
		private boolean first = true;
		
		public Object next() {
			// as long as the head is smaller than the tail it's not wrapped around. 
			if (head < tail) {
				notwrapped = false;
			}
			// this code runs when the array is wrapped around. 
			else {
				// if it's the count reaches the end of the array it should become the wrap around and start from there 
				if (count == size) {
					count = 0;
					first = false;
				}
		...
		public boolean hasNext() {
			// this return statement is when array is not wrapped around
			if (!notwrapped) {
				return count != tail;
			}
			// this is used when the array is not wrapped around.
			return count != tail+1 || first;
		}
